...
const AccountDisplay = ({
  ...
  let voteBox = <></>;
  if (hasCastedVote) {
    voteBox = <p className="text-brand-text-2">Your vote has been casted.</p>;
  } else if (hasAlreadyVoted) {
    voteBox = (
      <p className="text-brand-text-2">You already voted for this account.</p>
    );
  } else {
    voteBox = (
  ...
  return (
    <>
      <button
        className="bg-brand-card rounded flex p-4 my-4 justify-items-stretch items-stretch w-full transform duration-300 hover:scale-105 z-10 break-all"
        title="Auto-fill"
        onClick={async () => {
      ...
      >
        ...
        <div className="flex flex-col justify-center text-left">
          ...
          <span className="text-brand-text-3 text-xs mt-1">
      ...
      {showVoteBox && !hasAlreadyVoted && provider?.supportsVote && (
        <div className="mx-3 -mt-4 bg-brand-card p-3 rounded-b votebox">
          <h3 className="font-bold text-white">Does this account work?</h3>
