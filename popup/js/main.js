...
// Restore current choice of checking available accounts
  ...
  fetch("http://bugmenot.com/view/" + domain)
    ...
    .then((data) => {
      ...
      if (page.getElementsByClassName("account").length == 0) {
        document.getElementById("logins").innerText = chrome.i18n.getMessage(
          "popupNoAccounts"
        );
      } else {
        // Clear logins list
      ...
      for (const el of elements) {
        ...
        element.addEventListener("click", () => {
          // Set timeout if no content script available
          const noConnectionTimeout = setTimeout(() => {
            // Show alert
            document.getElementById("could-not-connect").style.display =
              "block";
          }, 100);
